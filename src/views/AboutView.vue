...
export default {
  data() {
    ...
    return {
      ...
      commands: {
        ...
        ls: {
          f: () => {
            self.term.writeln(['a', 'bunch', 'of', 'fake', 'files'].join('\r\n'));
            self.term.prompt(self.term);
          },
  ...
  mounted() {
    ...
    self.term = new Terminal({
      ...
      width: '100%',
      // height: '100%',
      cols: parseInt(window.innerWidth / 9), //any value
      rows: parseInt(window.innerHeight / 19.3), //any value
    });
  ...
  methods: {
    ...
    runFakeTerminal() {
      ...
      self.term.writeln('Below is a simple emulated backend, try running `help`.');
